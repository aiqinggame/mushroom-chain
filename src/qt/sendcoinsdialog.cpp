    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a MushroomChain address (e.g. MURTDp3LdR98Cufh9BbSSqUPJFEtPKSLSe)"));
#endif
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid MushroomChain address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
