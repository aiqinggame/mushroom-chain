...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MushroomChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MushroomChain
    // Mac: ~/Library/Application Support/MushroomChain
    // Unix: ~/.MushroomChain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MushroomChain";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MushroomChain.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty MushroomChain.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "MushroomChaind.pid"));
